...
def shapiro(energy, sysType, setn, setN,  label="Label", p_value=0.05, grace=40, verbose=2, save='shapiro'):
    """
    This code is slightly adjusted from the original paper, just so the format would match
    ...
    data = pd.DataFrame(data, columns=['Time', 'Temperature', 'Pressure', 'Volume', 'Density'])
...
def shapiro_test_plot(residuals, temperature, title):
    """Plot histogram and Q-Q plot of residuals."""
    fig, axs = plt.subplots(1, 2, figsize=(12, 5))

    # Histogram of residuals
    axs[0].hist(residuals, bins=30, alpha=0.7, color='blue')
    axs[0].set_title(f"{title} - Residual Histogram")
...
def Tg_finder(density, label="Label", p_value=0.05, grace=40, verbose=2, save='shapiro', sysType=None, setn=None, setN=None):
    """
    ...
    skip = 3 # points to start with

    if grace is None:
         grace = (Temperature[-1]-Temperature[0]) * 0.20 
         print(f"grace: {grace}")
    ...
    if verbose >= 1 or save is not None:
        ...
        #shapiro_test_plot(G_Density - [G_m*T+G_b for T in G_Temperature], G_Temperature, title="Glass Phase")
        #shapiro_test_plot(L_Density - [L_m*T+L_b for T in L_Temperature], L_Temperature, title="Liquid Phase")
    ...
    return Tg
os.chdir(r'D:\pythongraphs\results_pdes\pdes18100')
#os.chdir(r'D:\pythongraphs\random stuff dump\pdms10\results\rand5n28e8')
#data = pd.read_csv("energy001.xvg", columns=['Time', 'Temperature', 'Pressure', 'Volume', 'Density'])
Tg = shapiro("energycr0001.xvg", sysType='PDMS', setn='18', setN='100', label="Tg", verbose=2)
