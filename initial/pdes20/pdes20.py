...
class MapsProtocol:
  def setup(self, app, plugin, result, molecule, jobName):
    ...
    writer.setOption("coulombtype", "Cut-off")
    writer.setOption("coulombtypeComment", "   ; method for electrostatics")
    writer.setOption("cutoff-scheme", "group")
    writer.setOption("cutoff-schemeComment", "   ; use charge groups")
    ...
    writer.setOption("rlist", "1.2")
    writer.setOption("rlistComment", "   ; cutoff distance for short-range neighbour list (nm)")
...
project=dm.findProjectByName("pdes")
if project == None:
  QMessageBox.critical(app, "Note", "Project \"pdes\" not found.")
else:
